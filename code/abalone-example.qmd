...
H~0~: **pH does not effect abalone growth (change in shell surface area per day, mm^2^ d^-1^).**  
...
# creating a new object called abalone_clean from the abalone data frame
...
ggplot(data = abalone_clean, # starting with clean data frame
       aes(x = mean_ph, # xaxis is ph, and yaxis is change in shell area
           y = change_in_area)) +
  # first layer: points to represent individual abalones 
  geom_point(size = 4, 
             stroke = 1,
             fill = "firebrick4",
             shape = 21)
...
```{r abalone-model}
abalone_model <- lm(
  change_in_area ~ mean_ph, # formula: response variable ~ predictor variable
  data = abalone_clean      # data: clean abaolone data frame 
)
```
...
**Residuals seem homoscedastic (consistent across values) based on Residuals vs Fitted and Scale-Locations plots. Residuals seem normally distributed based on QQ-plot. No major outliers based on Resdiuals vs Leverage plot.**

Written Communication: **We evaluated models visually using. For homoscedasticity of residuals, we used Residuals vs Fitted and Scale-Location plots. For normality of residuals, we used QQ-plots. Based on our diagnostics, there were no major deviations from homoscedasticity or normality of residuals. There were also no outliers that might influence model estimates or predictions.**

Notes from An: The 2 plots Residuals vs Fitted and Scale-Location, tell us the same thing. That the residuals are the same, that the distance between the models predicted values and the actual observed values are the same across all values. 
QQ plot shows visually if the the data is normally distributed.
Residuals vs Leverage plot tells you that there might be an outlier (10).
...
**18.5 ± 6.2 (SE). When pH = 0, the model predicts that abalone change in shell area per day is -18.5. (Note to self: this is not biologically realistic, but is something the model generates).**
...
**predicted (model prediction for change in shell surface area per day)**
**std.error (standard error describinbg precision in model predictions)**
...
**At pH = 8, the model predicts that abalone change in  shell surface area per day should be 5.36 mm^2^ per day^-1^ (95% CI: 5.08, 5.64 mm^2^ day^-1^).**
